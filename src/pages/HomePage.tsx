...
function ScorerRow({ scorer, rank }: { scorer: PlayerStat; rank: number }) {
  return (
    <div className="flex items-center gap-2.5 py-2">
      ...
      <span className="text-base font-black text-primary tabular-nums">{scorer.goals}</span>
    </div>
  );
}
...
function StandingsWidget() {
  ...
  return (
    <div>
      ...
      {/* Header */}
      <div className="flex items-center gap-2 py-1.5 text-[10px] text-muted-foreground/60 uppercase font-bold">
      ...
      {current.teams.map(t => (
        <div key={t.rank} className="flex items-center gap-2 py-2 border-t border-border/10">
          ...
          <span className="text-sm font-black text-primary w-8 text-center tabular-nums">{t.pts}</span>
        </div>
      ))}
      {/* Dots */}
      {tables.length > 1 && (
