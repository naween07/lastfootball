              ...
              <div className="absolute right-0 top-full mt-1 w-64 bg-card border border-border rounded-xl shadow-xl overflow-hidden z-50">
                ...
                <div className="max-h-64 overflow-y-auto">
                  {filteredDropdown.length > 0 ? filteredDropdown.map(league => (
                    ...
                    >
                      <OptimizedImage src={league.logo} alt="" className="w-4 h-4 object-contain flex-shrink-0 rounded bg-white/10 p-[1px]" />
                      <span className={cn(
                        'text-xs truncate',
  ...
  return (
    ...
    >
      {logo?.startsWith('http') ? (
        <OptimizedImage src={logo} alt="" className="w-4 h-4 rounded bg-white/10 p-[1px]" priority={active} />
      ) : logo ? (
        <span>{logo}</span>
      ) : null}
      {label}
